...
ansible-vault encrypt "${KEY_ED25519}" --output "${FILE_DIR}/$(basename ${KEY_ED25519}).aes256"
...
ansible-vault encrypt "${KEY_RSA}" --output "${FILE_DIR}/$(basename ${KEY_RSA}).aes256"
HASH_RSA=$(sha256sum "${KEY_RSA}" | cut -d ' ' -f1)

# moduli
ansible-vault encrypt "${MODULI_SCREEN}" --output "${FILE_DIR}/$(basename ).aes256"
